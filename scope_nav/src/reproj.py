...
def reprojection_to_map(
    ...
    src_x_min, src_x_max = src_x_lim
    src_y_min, src_y_max = src_y_lim

    # 1) prob -> logit
    source_logit = torch.logit(source_map.clamp(eps, 1 - eps))

    # 2) target grid centers in MAP coords
    ix = torch.arange(out_h, device=device)
    iy = torch.arange(out_w, device=device)
    ix_grid, iy_grid = torch.meshgrid(ix, iy, indexing='ij')  # (H,W)

    x_tg = map_origin_x + (ix_grid.to(dtype) + 0.5) * resolution
    y_tg = map_origin_y + (iy_grid.to(dtype) + 0.5) * resolution

    x_tg = x_tg.unsqueeze(0).expand(B, -1, -1)  # (B,out_h,out_w)
    y_tg = y_tg.unsqueeze(0).expand(B, -1, -1)

    # 3) inverse transform: target(map) -> source(output/local)
    ct = torch.cos(dtheta).view(B, 1, 1).to(dtype)
    st = torch.sin(dtheta).view(B, 1, 1).to(dtype)
    dxv = dx.view(B, 1, 1).to(dtype)
    dyv = dy.view(B, 1, 1).to(dtype)
    ...
    # 4) source metric -> normalized coords for grid_sample
    grid_x = 2.0 * (y_src - src_y_min) / (src_y_max - src_y_min) - 1.0  # W-axis
    grid_y = 2.0 * (x_src - src_x_min) / (src_x_max - src_x_min) - 1.0  # H-axis

    grid = torch.stack([grid_x, grid_y], dim=-1)  # (B,out_h,out_w,2)

    valid = (grid_x >= -1.0) & (grid_x <= 1.0) & (grid_y >= -1.0) & (grid_y <= 1.0)
    valid_mask = valid.to(dtype).unsqueeze(1)  # (B,1,out_h,out_w)

    # 5) bilinear warp in logit space
