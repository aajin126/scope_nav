...
{
void CrowdLayer::onInitialize()
{
  ros::NodeHandle nh("~/" + name_), g_nh;
  current_ = true;
  first_time_ = true;
  std::string people_topic;
  nh.param("people_topic", people_topic, std::string("scope_prediction"));
  people_sub_ = nh.subscribe("/" + people_topic, 1, &CrowdLayer::peopleCallback, this);//nh.subscribe("/people", 1, &CrowdLayer::peopleCallback, this);
}

void CrowdLayer::peopleCallback(const people_msgs::People& people)
{
...
void CrowdLayer::updateBounds(double origin_x, double origin_y, double origin_z, double* min_x, double* min_y,
